  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 55.6 + 'px',
        nekoImg: "https://fairycn.github.io/assets/img/panda.png",
        hoverMsg: "Hello",
  ...
  if (localStorage.getItem("mouse") == "on") {
    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "Switch right button mode successfully🍔",
            message: "The current right mouse button has been restored to the system default！",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "success",
    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "Switch right button mode successfully🍔",
            message: "The current right mouse button has been changed to the style specified by the website！",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "success",
...
function createtime1() {
  var grt = new Date("02/28/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  var ascll2 = [`NCC2-036`, `Hello`, `Photo captured: `, ` 😊 `];
  ...
  setTimeout(console.log.bind(console, "%c WELCOME %c Is there any problem with the current blog operation", "color:white; background-color:#23c682", ""));

  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ 🍭%c You are troubleshooting the problem🍭",
      "color:white; background-color:#f0ad4e",
      ""
    )
  );

  setTimeout(console.log.bind(console, "%c W23-12 %c Opened the browser console", "color:white; background-color:#4f90d9", ""));
  setTimeout(
    console.warn.bind(console, "%c S013-782 %c Has the problem been solved", "color:white; background-color:#d9534f", "")
  );
}
createtime2();
      ...
      new Vue({
        data: function () {
          this.$notify({
            ...
            message: "Currently successfully switched to night mode！",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "success",
      ...
      new Vue({
        data: function () {
          this.$notify({
            ...
            message: "Currently successfully switched to daytime mode！",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "success",
...
function share_() {
  ...
  try {
    ...
    navigator.clipboard.writeText('fairycn Blog\n title：' + subTitle + '\nLink：' + url + '\n🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "Successfully copy and share information🎉",
          message: "You can now share it directly with your buddies by pasting.！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("02/28/2023 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://img.shields.io/badge/fairycn-blog-blue.svg' title='Welcome to my blog'><br> <div style="font-size:13px;font-weight:bold">Blog running ${dnum} day ${hnum} houer ${mnum} minute ${snum} second <i id="heartbeat" class='fas fa-heartbeat'></i> <br> Voyager 1 is currently ${dis} kilometers away from the earth, about ${unit} astronomical units 🚀</div>`
      : `<img class='boardsign' src='https://img.shields.io/badge/fairycn-blog-blue.svg' title='Welcome to my blog'><br> <div style="font-size:13px;font-weight:bold">Blog running ${dnum} day ${hnum} houer ${mnum} minute ${snum} second <i id="heartbeat" class='fas fa-heartbeat'></i> <br> Voyager 1 is currently ${dis} kilometers away from the earth, about ${unit} astronomical units 🚀</div>`),
...
if (window.localStorage.getItem("fpson") == undefined || window.localStorage.getItem("fpson") == "1") {
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        var kd = `<span style="color:#bd0000">Network speed🤢</span>`
      } else if (fps <= 15) {
        var kd = `<span style="color:red">Network speed😖</span>`
      } else if (fps <= 25) {
        var kd = `<span style="color:orange">Network speed😨</span>`
      } else if (fps < 35) {
        var kd = `<span style="color:#9338e6">Network speed🙄</span>`
      } else if (fps <= 45) {
        var kd = `<span style="color:#08b7e4">Network speed😁</span>`
      } else {
        var kd = `<span style="color:#39c5bb">Network speed🤣</span>`
      }
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(../assets/img/kanxue.jpg);
    --darkmode-bg:url(../assets/img//yuanshen1.webp);
    --mobileday-bg: url(../assets/img/snow.webp);
    --mobilenight-bg: url(../assets/img/mb8.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
<br>
<center><p><button onclick="reset()" style="background:linear-gradient(to right, #fc354c, #0abfbc);display:block;width:40%;padding:15px 0;border-radius:30px;color:white;font-size:1.1em;"><i class="fa-solid fa-arrows-rotate"></i>&nbsp;Restore default</button></p></center>

<h2>Display preference</h2>

<div class="transValue" style="font-weight:bold;padding-left:10px">Transparency (0%-100%): ${curTransNum}%</div>
<div class="range">
...
<div class="blurValue" style="font-weight:bold;padding-left:10px">Blur radius (enable blur effective 0px-100px): ${curBlur} px</div>
<div class="range">
...
<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> Starry sky (Night mode) </div><input type="checkbox" id="universeSet" onclick="setUniverse()">
  <div class="content-text" style="font-weight:bold; padding-left:20px"> Neon light (Night mode) </div><input type="checkbox" id="lightSet" onclick="setLight()">
</div>

<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> Fuzzy </div><input type="checkbox" id="blur" onclick="setBlur()">
  <div class="content-text" style="font-weight:bold; padding-left:20px"> Sidebar (On by default) </div><input type="checkbox" id="rightSideSet" onclick="toggleRightside()">
</div>

<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> Frame rate monitoring (Refresh takes effect) </div><input type="checkbox" id="fpson" onclick="fpssw()">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> snows (Daytime mode) </div><input type="checkbox" id="snowSet" onclick="setSnow()">
</div>


<h2>Font settin</h2>
<div class="note warning modern"><p>Non-commercial free fonts can only be used by individuals without authorization. This site is a completely non-commercial, non-profit website, usually used for personal learning and communication, if there is infringement, please contact the webmaster to delete, thank you! -to the copyright owner</p>
</div>
<p id="swfs">
...
<a class="swf" id="swf_default" href="javascript:;" rel="noopener external nofollow" style="font-family:-apple-system, IBM Plex Mono ,monosapce,'微软雅黑', sans-serif;!important;color:black" onclick="setFont('default')">System default</a>
</p>

<h2>Theme Color Settin</h2>
...
<h2>Background settin</h2>
<center><button onclick="resetBg()" style="background:var(--theme-color);display:block;width:35%;padding:15px 0;border-radius:30px;color:white;"><i class="fa-solid fa-arrows-rotate"></i>&nbsp;Restore the default background</button></center>

<h3>1. Two dimensions</h3>
<details class="folding-tag" cyan><summary> View Two dimensions background </summary>
              <div class='content'>
...
<details class="folding-tag" cyan><summary> View the landscape background </summary>
              <div class='content'>
...
<details class="folding-tag" cyan><summary> Check the cute background. </summary>
              <div class='content'>
...
<h3>4. Gradient color</h3>
<details class="folding-tag" cyan><summary> View gradient background </summary>
              <div class='content'>
...
<h3>5. Solid color</h3>
<details class="folding-tag" cyan><summary> View solid color background </summary>
              <div class='content'>
...
<h3>6. Phone</h3>
<details class="folding-tag" cyan><summary> Check the background of the adaptable phone. </summary>
              <div class='content'>
...
<h3>7. WallpaperAPI</h3>
<details class="folding-tag" cyan><summary> View the background of the wallpaper API series </summary>
              <div class='content'>
...
<h3>8. custom background</h3>
<details class="folding-tag" cyan><summary> Set custom background </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="Please enter a valid image link, such as https://source.fomal.cc/img/home_bg.webp"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>

<br>
<center><div style="font-size:1.2em;color:var(--theme-color);font-weight:bold;">------ ( •̀ ω •́ )y To the end. ------</div></center>
<br>
